...
trait ConfigServiceComponentImpl extends ConfigServiceComponent {
  trait ConfigServiceImpl extends ConfigService {

    val CONFIG_PROPS_FILE = "/config/config.properties"
    private lazy val props: Properties = read(CONFIG_PROPS_FILE)

    /*
     Reads System Java properties then properties file
